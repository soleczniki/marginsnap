...
- **Separate sign-up from sign-in** (2026-09-17, Bogdan's note) — the
  ...
  when there's time.
- **Shop currency change detection** (2026-09-18, Bogdan's decision) — Etsy
  locks a shop to one currency at a time, changeable shop-wide via Shop
  Manager → Payment Settings (14-day revert window); `Order.currency` above
  already stores the real per-order currency from each receipt, which is
  fine day-to-day, but if a seller ever changes their shop's currency,
  older orders stay recorded in the old one while new orders come in under
  the new one — inconsistent data that breaks graphing/reporting across
  periods. Bogdan's call on handling it: when a sync detects the shop's
  current Etsy currency no longer matches what's on record, MarginSnap
  should **tell** the seller (not ask) that all previous data will be
  switched/unified to the new current currency — not offered as a choice,
  since mixed currencies aren't a real option either way. Deliberately
  deferred — no detection, notification, or migration/backfill logic exists
  yet. Needs, when picked up: somewhere to snapshot "what's on record" (a
  new `Shop.currency` field set on sync, most likely), what triggers the
...
- ~~**Set `ANTHROPIC_API_KEY` in Vercel's env vars.**~~ Done — Bogdan added
  ...
  error after this, check whether the live deployment predates the env var
  being added — Vercel bakes env vars in at deploy time, so a redeploy is
  needed to pick up a var added after the last build. `ANTHROPIC_VISION_MODEL`
  is optional — only set it if the default model in `src/lib/adSpendVision.ts`
  ever needs swapping out.
